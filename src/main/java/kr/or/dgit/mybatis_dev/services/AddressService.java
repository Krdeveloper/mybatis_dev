...
public class AddressService {
	...
	public Address selectAddressByAddrIdAndStateForMap(Map<String, Object> param){
		try(SqlSession sqlSession = MybatisSqlSessionFactory.openSession()){
			AddressMapper tutorMapper = new AddressMapperImpl(sqlSession);
			return tutorMapper.selectAddressByAddrIdAndStateForMap(param);
		}
	}
	...
	public List<Address> selectAddressByAll(RowBounds rowBounds){
		try(SqlSession sqlSession = MybatisSqlSessionFactory.openSession()){
			AddressMapper tutorMapper = new AddressMapperImpl(sqlSession);
			return tutorMapper.selectAddressByAll(rowBounds);
		}
	}
}
